...
const Test = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8000/api/v1/modifications", { withCredentials: true })
      .then((response) => {
  ...
  return (
    <>
      ...
      {/* Settings Panel */}
      <div id="squarecraft-panel" className="hidden fixed bottom-20 right-5 bg-white p-4 shadow-lg rounded-md">
        ...
        <button onClick={handleApplyChanges} className="bg-blue-500 text-white px-4 py-2 rounded mt-2">
          Apply
        </button>
      </div>

      {/* Click event to select elements */}
      <div onClick={handleElementSelect} className="absolute top-0 left-0 w-full h-full"></div>
    </>
  );
};
